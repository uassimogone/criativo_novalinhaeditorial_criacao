...
class ImageResearcher:
    """Busca imagens abertas no Wikimedia Commons e falha com segurança para layout sem foto."""

    API_URL = "https://commons.wikimedia.org/w/api.php"
    USER_AGENT = "UassiNovaLinhaEditorial/1.0 (editorial image research)"
    MIN_WIDTH = 1400
    MIN_HEIGHT = 900
    ...
    def find_and_download(self, query: str, content_id: str, target_dir: Path) -> VisualAsset | None:
        if not query:
            return None
        for candidate in self._search(query):
            if not self._is_acceptable(candidate):
                continue
            asset = self._download(candidate, content_id, target_dir)
            if asset:
                return asset
        return None
    ...
    def _is_acceptable(cls, item: dict) -> bool:
        ...
        return (
            ...
            and item.get("width", 0) * item.get("height", 0) <= 70_000_000
    ...
    def _candidate_score(item: dict) -> float:
        ...
        ratio = width / height
        portrait_target = 4 / 5
        ratio_bonus = 4 - min(4, abs(ratio - portrait_target) * 3)
        resolution_bonus = min(4, (width * height) / 5_000_000)
        return ratio_bonus + resolution_bonus
